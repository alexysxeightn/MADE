...
def main(args):
    ...
    df = df.select(
        split(col("value"), '\t').getItem(0).alias("ts"),
    ...
    res = spark.sql("""
        SELECT
            window(to_timestamp(from_unixtime(ts)), '2 second', '1 second') as window,
            CASE WHEN parse_url(url, 'HOST') LIKE '%.ru' THEN 'ru' ELSE 'not ru' END AS zone,
            COUNT(uid) as view,
